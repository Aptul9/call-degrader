...
**The preview is smaller and slower than the call.** It is encoded 640 wide at 15 fps, and only while a page is showing it: with the window hidden to the tray or minimised, nothing is encoded at all. The virtual camera gets every frame at full size either way.
...
**The host API decides the delay as well as whether anything comes out.** A playback stream into `CABLE Input` negotiates 22 ms on WASAPI, 100 ms on MME asking for its low buffer, 180 ms on MME asking for nothing, and 240 ms on DirectSound. sounddevice asks for nothing by default, so the first release wrote into the cable through 180 ms of buffer and nobody looked at the number. Capture barely differs in delay, 20 ms on MME against 22 ms on WASAPI for the same microphone, but it differs in cadence: MME hands its 10 ms blocks over two at a time, and the queue between the two streams had to stand deep enough to absorb the pairs. So capture leads with WASAPI as well, with MME right behind it for a device whose WASAPI view will not start. The WASAPI-into-`CABLE Input` pairing recorded in this file as a segfault did not reproduce: a 0.3 tone written that way and read off `CABLE Output` on DirectSound from a second process arrived at peak 0.3001.
...
**The queue between the microphone and the cable has to be held at a depth, not just capped.** Two devices run on two clocks, so the audio passes through a queue, and nothing used to take a surplus back out of it: wherever it settled at start was the delay for the rest of the call, anywhere from 10 to 80 ms on top of the drivers, 60 to 70 ms on one measured run. It is now brought back to one standing block, a block at a time and crossfaded, and an empty queue fades the last block out instead of cutting to silence.

**pyvirtualcam converts the frame while holding the GIL.** Fed RGB it spent 5.5 ms a frame turning it into the NV12 the OBS driver takes, and nothing else in the process could run meanwhile, the audio callbacks included. It is fed I420 now, converted by OpenCV, which releases the GIL, and pyvirtualcam is left copying planes.

**A camera already paces the loop, just not evenly.** `read()` blocks until the next frame, so a sleep on a timer of the loop's own was a second clock, and it cost twice. When the two drifted apart the loop waited on both: a 21 ms sleep followed by a 29 ms wait for the frame the sleep had just missed went out as a 62 ms gap. When the loop ran late, frames queued behind the timer and stayed queued: by their capture timestamps they reached the virtual camera 60 to 112 ms old, depending on the run. Sent the moment they arrive they are 44 ms old, but MSMF hands them over anywhere from 16 to 49 ms apart although they were captured 33.3 ms apart, and that unevenness reached the far end as 6 to 16 gaps over 70 ms in 90 s. So each frame now goes out on the camera's own clock, at its capture time plus the transit that 95 in 100 recent frames made within: 60 ms on every run, with 0 to 4 such gaps, as smooth as the queued version was. A thread that reads the camera and hands a steady tick the newest frame was measured too and lost on both counts, 72 ms and 12 to 15 gaps, because the tick is still a second clock sampling the camera's. Only the generated sources sleep on a timer, with `time.sleep`, because `Event.wait` rounds up to the 15.6 ms system tick unless something in the process has raised the timer resolution.

**OpenCV's thread pool was most of the CPU.** By default it spreads every call over all eight cores and spins between calls, and on 720p frames that buys nothing worth the price: with the camera running and no effects, the whole app measured 28.1 percent of a core on the default pool against 15.9 on two threads. The effects are the only calls that use the extra threads, 6.7 to 9.5 ms a frame on eight against 7.8 to 11.2 on two, so the pool is set to two.

**Some lag is the machine, not the tool.** On the development machine, a Core Ultra 7 258V on battery, a bare process doing nothing but sleeping for 1 ms missed its wake-up by 16 to 130 ms several times a minute, in clusters a few seconds long, with this tool closed. Two such processes stalled at the same instants, and opting one out of Windows power throttling changed nothing. No buffer short of that size rides through it, the call client's own included.
